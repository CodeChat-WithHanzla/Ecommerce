...
function Navigation() {
    ...
    return (
        <div id='navigation-container' style={{ zIndex: 999 }} className={`${showSidebar ? "hidden" : "flex"} xl:flex lg:flex md:hidden sm:hidden flex-col justify-between p-4 text-white bg-black w-[4%] hover:w-[15%] h-[100vh] fixed`}>
            <button
                className="fixed top-4 left-4 z-[1000] text-white xl:hidden lg:hidden md:block sm:block"
            ...
            <div className="relative">
                <button onClick={toggleDropdown} className='flex items-center text-gray-800 focus:outline-none'>
                    {userInfo && (<span className='text-white -ml-1.5'>{userInfo?.username}</span>)}
                    {userInfo && (
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className={`w-5 h-5 ${dropdownOpen ? "transform rotate-180" : "none"}`}
                            viewBox="0 0 24 24"
                            stroke="white"
                        >
                            <path
                    ...
                    )}
                    {
                        dropdownOpen && userInfo && (
                            <ul className={`absolute right-0 mt-2 mr-14 space-y-2 bg-white text-gray-600 ${!userInfo.isAdmin ? "-top-20" : "-top-80"}`}>
                                ...
                                <li>
                                    <Link onClick={logoutHandler} to='/logout' className='block px-4 py-2 hover:bg-gray-100'>
                                        Logout
                                    </Link>
                                </li>
                            </ul>
